...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_BRAND: 'brand/PARTIAL_UPDATE_BRAND',
...
export default (state: BrandState = initialState, action): BrandState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_BRAND):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_BRAND):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_BRAND):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IBrand> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_BRAND,
